...
Deno.test("Format and Equal traits dispatch through pseudo-trait helpers", () => {
  assert_equals(Format.fmt(Option.some(42)), "Some(42)");
  assert_equals(Format.fmt(Option.none()), "None");
  assert_true(Equal.eq(Option.some("x"), Option.some("x")), "boxed same");
  assert_true(!Equal.eq(Option.some("x"), Option.some("y")), "boxed diff");

  const list = List.from_array([1, 2, 3]);
  assert_equals(Format.fmt(List(list)), "[1, 2, 3]");
  assert_true(
    Equal.eq(List(list), List(List.from_array([1, 2, 3]))),
    "boxed list",
  );

  assert_equals(Format.fmt(Result.ok("done")), 'Ok("done")');
  assert_equals(Format.fmt(Result.err("bad")), 'Err("bad")');
  assert_true(
    Equal.eq(Result.ok("done"), Result.ok("done")),
    "boxed result",
  );
});

Deno.test("Functor maps values without leaving the context", () => {
  const some = Functor.map(Option.some(2), (value: number) => value + 1);
  const none = Functor.map(
    Option.none<number>(),
    (value: number) => value + 1,
  );
  const list = Functor.map(
    List(List.from_array([1, 2, 3])),
  ...
  assert_equals(some.value(), Option.some(3).value());
  assert_equals(none.value(), Option.none().value());
  assert_equals(List.to_array(list.value()), [2, 4, 6]);
  assert_equals(ok.value(), Result.ok(21).value());
  assert_equals(err.value(), Result.err("missing").value());
});
...
Deno.test("Applicative applies contextual functions", () => {
  ...
  assert_equals(option.value(), Option.some(42).value());
  assert_equals(none.value(), Option.none().value());
  assert_equals(List.to_array(list.value()), [2, 3, 10, 20]);
});
...
Deno.test("Option callable wrapper traits option values for fluent methods", () => {
  ...
  assert_equals(value.value(), Option.some(42).value());
  assert_equals(none.value(), Option.none().value());
...
Deno.test("Option callable wrapper chains applicative ap through this", () => {
  const direct = Option.some((value: number) => value + 22)
    .ap(Option.some(20));
  ...
  assert_equals(direct.value(), Option.some(42).value());
  assert_equals(sum.value(), Option.some(42).value());
  assert_equals(missing.value(), Option.none().value());
});
...
Deno.test("Result callable wrapper derives fluent methods from its dictionary", () => {
  const value = Result.ok(40)
    .map((item) => item + 2);
  const parsed = Result.ok("42")
    .flat_map((text) => Result.from_number(Number.parseInt(text, 10)));
  const sum = Result.ok((left: number) => {
    return (right: number) => left + right;
  })
    .ap(Result.ok(40))
    .ap(Result.ok(2));
  const missing = Result.err<number>("missing")
    .map((item) => item + 1);

  assert_equals(value.value(), Result.ok(42).value());
  assert_equals(value.fmt(), "Ok(42)");
  assert_true(value.eq(Result.ok(42)), "static result compares");
  assert_true(value.eq(Result.ok(42)), "wrapped result compares");
  assert_equals(parsed.value(), Result.ok(42).value());
  assert_equals(parsed.fmt(), "Ok(42)");
  assert_equals(sum.value(), Result.ok(42).value());
  assert_equals(missing.value(), Result.err("missing").value());
});
...
Deno.test("List callable wrapper derives fluent methods from its dictionary", () => {
  ...
  assert_equals(values.value(), List.from_array([2, 4, 6]).value());
...
Deno.test("Monad chains computations that choose the next context", () => {
  ...
  const kept = Monad.flat_map(Option.some(4), positive);
  const dropped = Monad.flat_map(Option.some(-1), positive);
  const parsed = Monad.flat_map(
    Result.ok("42"),
    (text: string) => Result.from_number(Number.parseInt(text, 10)),
  );

  assert_equals(kept.value(), Option.some(4).value());
  assert_equals(dropped.value(), Option.none().value());
  assert_equals(parsed.value(), Result.ok(42).value());
});
...
Deno.test("Generic helpers work against trait interfaces", () => {
  const option = label_values(Option.some(5));
  const list = label_values(List(List.from_array([1, 2])));
  const result = label_values(Result.ok(3));

  assert_equals(option.value(), Option.some("value:5").value());
  assert_equals(List.to_array(list.value()), ["value:1", "value:2"]);
  assert_equals(result.value(), Result.ok("value:3").value());
  assert_equals(sum_values(List(List.from_array([1, 2, 3]))), 6);
});

Deno.test("Generic applicative helper combines contextual values", () => {
  const option = add_values(Option.some(20), Option.some(22));
  const none = add_values(Option.some(20), Option.none<number>());
  const result = add_values(Result.ok(40), Result.ok(2));
  const err = add_values(Result.err<number>("missing"), Result.ok(2));
  ...
  assert_equals(option.value(), Option.some(42).value());
  assert_equals(none.value(), Option.none().value());
  assert_equals(result.value(), Result.ok(42).value());
  assert_equals(err.value(), Result.err("missing").value());
  assert_equals(List.to_array(list.value()), [3, 21, 12, 30]);
});
...
Deno.test("Generic monad helper lets each context define failure", () => {
  ...
  const list = keep_positive(
    List(List.from_array([2, -1, 3])),
    () => List(List.nil()),
  );

  assert_equals(option.value(), Option.none().value());
  assert_equals(result.value(), Result.err("negative: -1").value());
  assert_equals(List.to_array(list.value()), [2, 3]);
});
